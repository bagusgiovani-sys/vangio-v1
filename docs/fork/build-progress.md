...
**Next action:** Live end-to-end pass of the TUI as v1 acceptance (banner, Tab cycle, Gryphon agents responding on DeepSeek via the migrated config, theme intact), and investigate the project-plugin startup stall (errors.md watchlist) since it blocks CLI commands in this repo. Then decide: ship/tag v1 vs start a post-v1 item.
